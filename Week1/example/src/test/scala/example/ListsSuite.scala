 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of list with only 1 element") {
    assert(sum(List(5)) === 5)
  }

  test("sum is negative") {
    assert(sum(List(1,2,3,0,-4,-5)) === -3)
  }
  ...
  test("max on negatives") {
    assert(max(List(-1,-9,-4,-2)) === -1)
  }

  test("max with repeated values") {
    assert(max(List(1,2,3,3,5,5,4)) === 5)
  }

  test("max with negative/positive/zero values") {
    assert(max(List(3,-7,0,4,-6)) === 4)
  }

  test("max throw NoSuchElementException on empty list") {
